...
# 依策略分配資金（總資金 500萬）
# 配置原則：效率加權（日報酬率高的策略獲得更多資金），
#           同時確保四策略全滿倉時總佔用 ≤ 500 萬
# 效率排序：D(128%) > C(120%) > B(93%) > A(74%)  [年化報酬/在場天]
# 滿倉驗算：A 10萬×8 + B 30萬×4 + C 25萬×6 + D 25萬×6 = 500萬
# 實際均值：~239萬（47.7%），P95：425萬（85%），峰值：450萬（90%）
CAPITAL_BY_STRATEGY: dict[str, int] = {"A": 100_000, "B": 300_000, "C": 250_000, "D": 250_000}
